...
namespace CloudMineSDKNUnitTests
{
	...
	public class CMAppObjDeleteTests
	{
		...
		public void SetObjectAndDeleteWithKey ()
		{
			...
			Task<CMObjectResponse> objResponse = appObjSrvc.SetObject (hcp, null, hcp.ID);
			objResponse.Wait ();
			...
			Task<CMObjectResponse> delResponse = appObjSrvc.DeleteObject (hcp.ID);
			delResponse.Wait ();

			Assert.AreEqual (delResponse.Result.Status, HttpStatusCode.OK);
			Assert.That (delResponse.Result.HasErrors, Is.False);
			Assert.That (delResponse.Result.Success.ContainsKey(hcp.ID), Is.True);
			Assert.AreEqual (delResponse.Result.Success [hcp.ID], "deleted");
		}
		...
		public void CanHandleMixedDeleteResults ()
		{
			...
			Task<CMObjectResponse> delResponse = appObjSrvc.DeleteObjects (new string[] {hcp.ID, "NotARealID"});
			delResponse.Wait ();
